...
const CameraCapture: React.FC<CameraCaptureProps> = ({ onCapture, onClose }) => {
  ...
  const capture = () => {
    if (videoRef.current && canvasRef.current) {
      ...
      if (step === 'activity') { setActivityImg(data); setStep('selfie'); } else { setSelfieImg(data); setStep('review'); }
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col items-center justify-between p-6 safe-area-inset">
      <div className="w-full flex justify-between items-center text-white"><h2 className="text-xl font-bold">{step === 'activity' ? 'Capture Proof' : step === 'selfie' ? "Proof it's You" : 'Review PACT'}</h2><button onClick={onClose} className="p-2 bg-zinc-800 rounded-full"><X size={24} /></button></div>
      <div className="relative w-full max-w-sm aspect-[3/4] bg-zinc-900 rounded-2xl overflow-hidden border-2 border-zinc-700 shadow-2xl">
        {step !== 'review' ? <video ref={videoRef} autoPlay playsInline muted className={`w-full h-full object-cover ${step === 'selfie' ? 'scale-x-[-1]' : ''}`} /> : <div className="relative w-full h-full"><img src={activityImg!} className="w-full h-full object-cover" alt="Activity" /><div className="absolute bottom-3 right-3 w-20 h-20 rounded-xl overflow-hidden border-2 border-black shadow-lg"><img src={selfieImg!} className="w-full h-full object-cover" alt="Selfie" /></div></div>}
        <canvas ref={canvasRef} className="hidden" />
      </div>
      <div className="w-full max-w-sm flex flex-col gap-4 items-center">
        {step === 'review' ? <div className="flex gap-4 w-full"><button onClick={reset} className="flex-1 py-4 bg-zinc-800 text-white rounded-xl font-bold flex items-center justify-center gap-2"><RefreshCw size={20} /> Retake</button><button onClick={() => onCapture(activityImg!, selfieImg!)} className="flex-1 py-4 bg-emerald-600 text-white rounded-xl font-bold flex items-center justify-center gap-2"><CheckCircle2 size={20} /> Submit</button></div> : <button onClick={capture} className="w-20 h-20 bg-white rounded-full border-8 border-zinc-700 flex items-center justify-center active:scale-95 transition-transform"><div className="w-14 h-14 bg-white border-2 border-zinc-900 rounded-full" /></button>}
...
const VerificationScreen: React.FC = () => (<div className="fixed inset-0 bg-black flex flex-col items-center justify-center p-12 text-center space-y-10 z-[60]"><div className="relative"><ShieldCheck size={80} className="text-emerald-500 animate-pulse" /><div className="absolute inset-0 border-4 border-emerald-500/20 rounded-full animate-ping" /></div><div className="space-y-4"><h2 className="text-3xl font-black italic uppercase tracking-tighter">AI Verification...</h2><p className="text-[10px] text-zinc-600 font-black uppercase tracking-[0.3em]">Neural analysis of Proof-of-Work in progress</p></div></div>);
...
export const CheckInPage: React.FC = () => {
  ...
  const handleCapture = async (activityImg: string, selfieImg: string) => {
    ...
    const selectedGoal = goals.find((g) => g.id === selectedGoalId); if (!selectedGoal) return;
    ...
    try {
      ...
      navigate('/');
    } catch (e) { console.error('Check-in error:', e); setIsVerifying(false); }
  };
